...
DCS-BIOS Control Panel using a WCH CH32V307VTC compatible dev board.  Sends commands via UDP over an integrated ethernet port.  Commands operate cockpit controls within Eagle Dynamic's DCS World.
...
It's purpose is to interface hardware (switches, knobs, rotary encodes, etc..) from DIY cockpit panels for use with Dynamic Eagle's Digital Combat Simulator (DCS) flight simumlator (see https://www.digitalcombatsimulator.com) via DCS-BIOS (see https://github.com/DCS-Skunkworks/dcs-bios and  https://gitmemories.com/DCS-Skunkworks/dcs-bios).
...
This is different than the CH32V307VCT6 dev boards from WCH. See example at https://www.amazon.com/MARALANG-Evaluation-CH32V307VCT6-CH32V307V-EVT-R1-500709463/dp/B0D768JL1N.
...
I sorry if explaining the configuration below seem redudant, obvious or long winded.  Go ahead an jump into it as you see fit.  I've come across too many projects on Github that don't explain how they work or how to use it. They assume the reader is well versed in the subject matter.  This is for those who want to get up and running without reverse engineering the project to make simple changes.  One configuration file is all that needs to be edited.
...
![WCH-LINKE](WCH-LINKE.png)

A configuration file (dcs_config.ini.inc) is used to define the network settings, encoder and button pin assignments along with their associated message to send to DCS-BIOS.  Encoders send a message on each click. Message is different for clockwise and counter-clockwise.  Buttons send a message when pressed and a different message when released.  The file is in ".ini" format.  It is compiled into the program.  So you will have to recompile and upload the firmware everytime you make a change.
...
"max_text_length" set the maximum length of the commands being sent.  Using a program called "bort" to see as many possible commands that can be sent, I did not find any longer that 48.  This keeps packet size to a minimum.  The entire command will fit within one packet.  I chose to make it an adjustable parameter in case a future command exceeds this length.  Rather than rewrite the code to adjust for it, we can change it here.

Bort (Bios Reference Tool) can be used to find just about any command you need to control DCS.  Not all commands are implimented by DCS-BIOS.  You can even send signals for testing.  It can be downloaded at "https://github.com/DCS-Skunkworks/Bort".
...
Network setting are used to configure how the device connects to your network.  Below is a sample network configuration.  "dhcp" is not implemented.  Could not get it to work.  Combining it with the UDP functions used too much memory and would error out saying so (error 11).  Maybe in the future.  If you figure out how to do it, please let me know.

"use_dcs_ip", when set to 1, will have the device send the commands directly to the IP address of the computer running DCS with DCS-Bios.  The addres is set in "dcs_ip".  If "use_dcs_ip" is set to 0 (zero), commands are sent to the broadcast address of your network.  It is calculated from its IP address and net mask.
...
Buttons are configured in a similar fashion, but have only three parameters each  plus a count to define the number of buttons total.  Buttons are momentary contact, but can be toggle switches.  A message is sent when it is press or turned on and a separate message when released or turned off.

Example code below defines one button, button 1. Button zero is reserved as a "SEND STATE" button. When pressed it will transmit the current state/position of all buttons.  You can assign it to any GPIO pin that can be assigned to buttons.  Default is PB3.  This is the user button on the device.  Count must include the SEND STATE button, too.  

Below assigns button 1 input to pin "PE0".  When pressed/closed it will send the message "LDG_LIGHT_SW 1" and "LDG_LIGHT_SW 0" when released/opened.

	"[buttons]\r\n"
	"count = 2\r\n"
	"btn0_pin = PB3\r\n"
	"btn0_press_text = SEND_STATE\r\n"
	"btn0_release_text = SEND_STATE\r\n"
	"btn1_pin = PE0\r\n"
	"btn1_press_text = LDG_LIGHT_SW 1\r\n"
	"btn1_release_text = LDG_LIGHT_SW 0\r\n"

DCS-BIOS also has togle commands.  For example "LDG_LIGHT_SW TOGGLE".  When using these commands, set only one parameter for momentary contact switches.  Leave the other blank/empty.  If you set both to toggle, it will enable the feature when closing the switch and immediately disable it when releasing the switch. With toggle switches you will set both messages to toggle.  It will flip the state when you close the switch and flip it back when you open it.  If the the state of the switch, in the game, when you first load up does not match the physical position of you toggle switch you can press the SEND STATE button or switch it in game once to match your physical position.  See examples below:

For momentary contact switches:

	"btn1_press_text = LDG_LIGHT_SW TOGGLE\r\n"
	"btn1_release_text = \r\n"

For toggle switches:

	"btn1_press_text = LDG_LIGHT_SW TOGGLE\r\n"
	"btn1_release_text = LDG_LIGHT_SW TOGGLE\r\n"
